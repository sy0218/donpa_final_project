...
try:
    previous_df = spark.read.option("header", "true").csv("hdfs:///user/ubuntu/donpa_data8.csv/*.csv")
except:
    previous_df = None
...
if previous_df is not None:
    ...
    # CSV 파일로 저장
    merged_df.write.csv("new_donpa8.csv", header=True, mode="overwrite")

    # 기존 디렉토리 삭제
    os.system("hdfs dfs -rm -r donpa_data8.csv")

    # 새로운 디렉터리 이름 변경
    os.system("hdfs dfs -mv new_donpa8.csv donpa_data8.csv")
else:
    # grouped_df로 바로 저장
    grouped_df.write.csv("donpa_data8.csv", header=True, mode="overwrite")
