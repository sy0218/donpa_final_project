...
# 파라미터 설정
params = {
    "itemName": "초전도 에너지 코어 상자[1회 교환가능]",
    "wordType": "match",
    "wordShort": "false",
    "limit": 10,
...
try:
    previous_df = spark.read.option("header", "true").csv("hdfs:///user/ubuntu/donpa_data7.csv/*.csv")
except:
    previous_df = None
...
if previous_df is not None:
    ...
    # CSV 파일로 저장
    merged_df.write.csv("new_donpa7.csv", header=True, mode="overwrite")

    # 기존 디렉토리 삭제
    os.system("hdfs dfs -rm -r donpa_data7.csv")

    # 새로운 디렉터리 이름 변경
    os.system("hdfs dfs -mv new_donpa7.csv donpa_data7.csv")
else:
    # grouped_df로 바로 저장
    grouped_df.write.csv("donpa_data7.csv", header=True, mode="overwrite")
